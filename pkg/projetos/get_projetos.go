...
func (h handler) GetProjetos(c *gin.Context) {
	...
	sql := "select pr.id_projeto, pr.nome_projeto, pr.equipe_id, eq.nome_equipe, pr.status, pr.data_inicio from projetos as pr inner join equipes as eq on pr.equipe_id = eq.id_equipe order by id_projeto"
