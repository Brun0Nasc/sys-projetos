...
`/equipes/` A função de _GET_ geral retorna os dados básicos de equipe:
...
`/equipes/1/` A função específica de _GET_ retorna junto aos dados de equipe, os membros dela:
...
`/equipes/1/projetos/` Retorna um _GET_ dos projetos da equipe especificada:
...
`/equipes/` Para dar _POST_ em uma nova equipe, a estrutura será a seguinte:
...
`/equipes/1/` Para atualizar equipes através do _PUT_, o único dado mutável será o nome da equipe:
...
`equipes/1/` Para excluir uma equipe através do _DELETE_ só será necessário passar o id da equipe na rota, a resposta será a seguinte:
...
`/pessoas/` A função de _GET_ geral retorna os dados básicos de pessoas:
...
`/pessoas/5/` O _GET_ específico retorna, além dos dados básicos de pessoas, as tasks da pessoa especificada:
...
`pessoas/` Para dar _POST_ e adicionar uma nova pessoa, a seguinte estrutura é necessária:
...
`pessoas/21/` Para atualizar uma pessoa através do _PUT_, os dados serão os mesmos que o _POST_:
...
`pessoas/1/favoritar/` Para favoritar uma pessoa, nenhum dado precisará ser passado, apenas a rota deve ser acessada com o método _PUT_, se a pessoa estiver favoritada, o câmbio entre favoritado e desfavoritado é automático.
...
`pessoas/1/` Para deletar uma pessoa através do método _DELETE_, apenas o id será necessário na rota, a resposta, caso dê certo, é a seguinte:
...
`/projetos/` `/projetos/1/` As funções _GET_ retornam os seguintes dados:
...
`/projetos/` Para dar _POST_ em um novo projeto, a estrutura será a seguinte:
...
`/projetos/1/` Para dar _PUT_ nas informações gerais de um projeto, a estrutura será a mesma que a de _POST_:
...
`/projetos/1/status/` Para dar _PUT_ atualizando o status do projeto, a única informação necessária será a de status:
...
`/projetos/1/` Para deletar através do _DELETE_, apenas o id do projeto que será deletado precisa ser passado, se for deletado corretamente, a resposta será a seguinte:
...
`/tasks/` `/tasks/1/` As funções _GET_ retornam as seguintes informações de tasks:
...
`/tasks/2/comentarios/` A rota _GET_ de comentários lista os comentários por Task:
...
`/tasks/` `/tasks/:id/` Para dar _POST_ ou _PUT_ em tasks, os seguintes dados serão necessários:
...
`/tasks/1/comentarios/` `/tasks/1/comentarios/1` Para o _POST_ de um novo comentário ou o _PUT_ de um comentário existente, a estrutura é a seguinte:
...
`/tasks/5/status/` Para atualizar o status da task, através do _PUT_ deverá ser passado o seguinte JSON:
...
`/tasks/5/` Para deletar uma task, através do _DELETE_ só será necessário acessar a rota com o id da task que deverá ser excluída:
...
`/tasks/1/comentarios/1` Para deleter um comentário na função _DELETE_, o id do comentário que será deletado deverá ser passado na rota:
```json
{
  "message": "Cadastro deletado com sucesso"
}
